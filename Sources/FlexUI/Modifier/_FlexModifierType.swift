...
/// 定义修改 Flex 布局的协议。
///
/// `_FlexModifierType` 协议用于修改符合 `_FlexView` 和 `_FlexDefinable` 的视图，通常与 `FlexModifiedContent` 结构体配合使用，来实现对布局的扩展和修改。
public protocol _FlexModifierType {
    /// 修改给定的 Flex 布局视图。
    ///
    /// - Parameter element: 需要被修改的视图，必须遵循 `_FlexView` 和 `_FlexDefinable` 协议。
    /// - Returns: 修改后的视图实例，仍然遵循 `_FlexView` 和 `_FlexDefinable` 协议。
...
public struct FlexModifiedContent<Content: _FlexViewType, Modifier: _FlexModifierType>: _FlexViewType {
    ...
    /// - Returns: 修改后的视图。
    private func modify(element: any _FlexView & _FlexDefinable) -> any _FlexView & _FlexDefinable {
