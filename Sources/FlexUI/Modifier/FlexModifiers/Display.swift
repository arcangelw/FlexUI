...
///
/// `FlexDisplayModifier` 允许通过 Flex 布局系统设置视图的显示属性。通过实现 `_FlexModifierType`，它可以应用到任何遵循 `_FlexView` 和 `_FlexDefinable` 的视图上。
public struct FlexDisplayModifier: _FlexModifierType {
    ...
    /// 初始化方法。
    ///
    /// - Parameter value: `Flex.Display` 值，表示视图的显示方式。
...
public extension _FlexViewType where Self: _FlexDisplayModifierType {
    ...
    ///
    /// - Parameter value: `Flex.Display` 值，指定视图的显示方式。
    /// - Returns: 包含应用了显示修饰符的视图内容。
    @discardableResult
